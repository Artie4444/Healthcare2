...
contract Agent {
    ...
    function get_nurse(address addr) public view returns (string memory , uint){
        // if(keccak256(nurseInfo[addr].name)==keccak256(""))revert();
        return (nurseInfo[addr].name, nurseInfo[addr].age);
    }
    ...
    function get_nurse_patient_name(address naddr, address paddr) view public returns (string memory , string memory ){
        return (nurseInfo[naddr].name,patientInfo[paddr].name);
    }
